...
const NovaSaidaModal = ({ onClose, onSave }: NovaSaidaModalProps) => {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-md flex items-center justify-center z-50">
      <div className="bg-card rounded-xl shadow-2xl p-8 w-full max-w-md max-h-[85vh] overflow-y-auto border border-border">
        ...
        <div className="space-y-6">
          <div>
            ...
            <div className="relative">
              <select
                ...
                className="w-full p-3 bg-background border border-input rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500 appearance-none"
              >
              ...
              <FaChevronDown className="absolute right-3 top-1/2 transform -translate-y-1/2 text-muted-foreground" />
            </div>
          </div>
          <div className="relative" ref={dropdownRef}>
            ...
            <input
              ...
              className="w-full p-3 bg-background border border-input rounded-lg text-foreground focus:outline-none focus:ring-2 focus:ring-indigo-500"
            />
            {isDropdownOpen && armazemId && (
              <ul className="absolute z-20 w-full mt-2 bg-card border border-input rounded-lg shadow-xl max-h-48 overflow-y-auto">
                {produtosLoading ? (
                  ...
                  filteredProdutos.map((produto) => (
                    <li
                      ...
                      className="p-3 hover:bg-indigo-50 cursor-pointer text-foreground text-sm transition-colors duration-150"
                    >
          ...
          <div>
            ...
            {saidaProdutos.length === 0 ? (
              ...
              <ul className="space-y-3 max-h-48 overflow-y-auto border-t border-border pt-3">
                {saidaProdutos.map((item) => (
                  <li
                    key={item.sku}
                    className="flex justify-between items-center p-3 bg-muted rounded-lg shadow-sm hover:bg-muted transition-colors duration-150"
                  >
          ...
          <div className="flex justify-end space-x-3">
            <button
              onClick={onClose}
              className="px-4 py-2 bg-muted text-foreground rounded-lg hover:bg-muted transition-colors duration-200"
            >
              Cancelar
            </button>
            <button
